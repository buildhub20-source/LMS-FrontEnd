      ...
      >
        ...
        >
          <svg viewBox="0 0 256 256" width="48" height="48">
            <path
              fill="currentColor"
              d="M128,12L24,52L36,192L128,244L220,192L232,52L128,12Z"
              opacity="0.1"
            />
            <path
              fill="currentColor"
              d="M128,24L40,58L48,186L128,230L208,186L216,58L128,24Z"
              opacity="0.2"
            />
            <path
              fill="currentColor"
              d="M128,34L196,182L162,182L146,144L110,144L94,182L60,182L128,34ZM128,78L118,124L138,124L128,78Z"
            />
          </svg>
        </div>
        ...
        >
          <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
            ...
            <div>
              ...
              <p style={{ margin: '0', fontSize: 12, color: 'var(--text-muted)' }}>Instructor</p>
            </div>
          </div>
...
export const AdminCourseListPage = () => {
  ...
  const handleAction = (type, course) => {
    ...
    if (type === 'reject') {
      setRejectModal({ course, loading: false });
      setRejectReason('');
      return;
    }
    const MAP = {
    ...
    if (MAP[type])
      setConfirmAction({
        ...
        action: type,
        loading: false,
        fn: () => doAction(course, type, MAP[type].fn, MAP[type].msg),
      });
  };
  ...
  const handleReject = async () => {
    ...
    try {
      await courseService.reject(rejectModal.course.id, { reason: rejectReason || null });
  ...
  const handleCreate = async () => {
    ...
    try {
      await courseService.create({
        ...
        level: createLevel,
        durationMinutes: createDuration ? parseInt(createDuration) : undefined,
      });
    ...
    <div className="space-y-6" style={{ fontFamily: 'system-ui, -apple-system, sans-serif' }}>
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <PermissionGuard required={[PERMISSIONS.COURSE_CREATE]} fallback={null}>
          <AdminButton icon={<Plus className="h-4 w-4" />} onClick={() => setCreateModal(true)}>
            New Course
          </AdminButton>
        </PermissionGuard>
      </div>
            ...
            {[
              ...
              <button
                ...
                style={{
                  ...
                  display: 'flex',
                  alignItems: 'center',
                  background: view === id ? 'var(--surface-medium)' : 'transparent',
        ...
        <AdminModal open title="Reject Course" onClose={() => setRejectModal(null)}>
          <div style={{ padding: '0 20px 20px' }}>
            ...
            <textarea
              ...
              rows={3}
              placeholder="Tell the creator why…"
              style={f}
            />
            <div style={{ display: 'flex', gap: 12, justifyContent: 'flex-end', marginTop: 16 }}>
              <AdminButton variant="ghost" onClick={() => setRejectModal(null)}>
                Cancel
              </AdminButton>
              <AdminButton variant="danger" loading={rejectModal.loading} onClick={handleReject}>
                Reject
              </AdminButton>
            </div>
          </div>
        ...
        <AdminModal open title="Create Course" onClose={() => setCreateModal(false)}>
          ...
          >
            ...
            <div style={{ display: 'flex', gap: 12, justifyContent: 'flex-end', marginTop: 8 }}>
              <AdminButton variant="ghost" onClick={() => setCreateModal(false)}>
                Cancel
              </AdminButton>
              <AdminButton
