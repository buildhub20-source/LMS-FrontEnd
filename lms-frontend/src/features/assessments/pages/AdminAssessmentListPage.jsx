  ...
  const handleAction = async (type, assessment) => {
    switch (type) {
      ...
      case 'close':
        if (
          !window.confirm(
            `Close "${assessment.title}"? Students will no longer be able to start new attempts.`,
          )
        )
          return;
        try {
        ...
        break;
      case 'archive':
        if (!window.confirm(`Archive "${assessment.title}"? It will be hidden from all listings.`))
          return;
        try {
        ...
        break;
      case 'delete':
        if (!window.confirm(`Permanently delete "${assessment.title}"? This cannot be undone.`))
          return;
        try {
  ...
  return (
    <PageContainer
      ...
      actions={
        <Button
          ...
          onClick={() => navigate(ROUTES.ADMIN_ASSESSMENT_CREATE)}
        >
          New assessment
        </Button>
      }
    >
      ...
      <Pagination
        ...
        totalPages={
          data?.totalPages ??
          Math.max(1, Math.ceil((data?.totalElements ?? 0) / pagination.pageSize))
        }
